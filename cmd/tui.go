...
func performDelete(worktreePath, worktreeName, branchName string, force bool, deleteBranch bool) tea.Cmd {
	return func() tea.Msg {
		...
		var err error
		if force {
			err = deps.Git.RemoveWorktree(worktreeName, &worktreePath, true)
		} else {
			err = deps.Git.RemoveWorktree(worktreeName, &worktreePath, false)
		}
		...
		if deleteBranch {
			...
			err = deps.Git.DeleteBranchRef(branchName, deps.AppConfig.BareRepoPath)
			if force {
				err = deps.Git.DeleteBranch(branchName, deps.AppConfig.BareRepoPath, true)
			} else {
				err = deps.Git.DeleteBranch(branchName, deps.AppConfig.BareRepoPath, false)
			}
			if err != nil {
...
func (m tuiModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
		...
		case tea.KeyMsg:
			...
			case "enter", "o":
				...
				if item, ok := selected.(popupItem); ok {
					...
					deps.Connector.SeshConnect(item.title)
					return m, tea.Quit
				}
				m.showPopup = false
				return m, nil
	...
	case tea.KeyMsg:
		...
		case "o":
			...
			zoxideEntries, err := services.GetQueryList(deps.Zoxide, selectedRow[2])
